...
class commandes(cmd.Cmd):
    prompt = '> '
    def do_readconfig(self, arg):
        main_analysis.read_config()

    def do_systemexec(self, syscmd):
        main_analysis.systemexec(syscmd)

    def do_selectlayer(self, arg):
        main_analysis.selectlayer(arg)

    def complete_selectlayer(self, text, line, start_index, end_index):
        #print("Text: {}".format(text))
        #print("Line: {}".format(line))
        print("layer : {}".format(main_analysis.get_layers()[0].startwith(text)))
        if text:
            print("toot")
            for i in main_analysis.get_layers():
                print("layer : {} ({})" % (i,type(i)))
            return [
                layer for layer in main_analysis.get_layers() 
                if layer.startwith(text)
            ]
        else:
            return main_analysis.get_layers()
